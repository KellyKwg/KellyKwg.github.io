...
// Game Over // 
setTimeout(function(){
  window.location.href = 'medend.html';
}, 182000);
...
function shuffle() {
  ...
  for (var row=1;row<=4;row++) { //For each row of the ...x... grid
     for (var column=1;column<=4;column++) { //For each column in this row
    
      var row2=Math.floor(Math.random()*4 + 1); //Pick a random row from 1 to (3,4,5,6)
      var column2=Math.floor(Math.random()*4 + 1); //Pick a random column from 1 to (3,4,5,6)
